...
以下を読み込む:
- `foundations/prohibited.md` の「AI生成パターンの排除」セクション
- `CLAUDE.md` の禁止パターン要約テーブル
...
#### 3-1. `foundations/prohibited.md` — SSOT（正規版）
...
パターンが特定のコンポーネントに関連する場合、該当する `components/*.md` の禁止事項セクションにも追記する。
...
### 更新ファイル
- foundations/prohibited.md — AI生成パターンの排除セクションに追加
- CLAUDE.md — 禁止パターン要約に追加
- [components/xxx.md — 禁止事項に追加]（該当する場合）
...
| 問題 | 対処 |
|------|------|
| `foundations/prohibited.md` が存在しない | ユーザーに報告し、ファイル作成の承認を得る |
