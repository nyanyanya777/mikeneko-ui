...
| ボタンの明示的な左寄せ配置（`justify-start` / `text-left`） | 日本の UI 慣習では確認・送信ボタンはコンテナの右側に配置する。左寄せのボタンはユーザーの期待する位置と異なり、操作の完了感が低下する | ボタンは右寄せ（`justify-end` / `ml-auto`）または中央寄せで配置する。ダイアログ・フォームの確認ボタンは右端に置く |

> **注意**: この禁止は `<Button>` コンポーネントの配置のみが対象。ボタン以外の要素のレイアウトには適用しない。
